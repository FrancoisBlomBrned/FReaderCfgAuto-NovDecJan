...
namespace Mapping
{
    public partial class ToString
    {
        ...
        */
        ...
        /*  commented 2023-04-20 (by Jason) (IPM-es5  不存在 SupplierCalibrationSoftwareVersion DID) */ // uncommented (Jan-26)
        ...
        */
        ...
        /*  2023-04-20   Jason,   IPM 不需要 ReadHeaterCurrent 的DID读取功能*/   // Uncommented (Jan-26)
    ...
    public static class UserArrays
    {
        ...
        internal static class Decoder
        {
            ...
            internal /*public*/static String? GetDecodesStrByteByByte(byte[] msgToDecode, Encoding[,] decodeReg /* = new byte[128,8]*/, string[,] addObjmsgRef /* = new byte[16,8]*/)
            {
                ...
                #region addByJ(对于全部输出为 ascii 情形， 增加的内容)
                try
                {
                    int max = msgToDecode.Length > 96 ? 96 : msgToDecode.Length;
                    string addMsgAscii = string.Empty + "(<)";
                    for (int msgIdx = 0; msgIdx < max; msgIdx++)
                    {
                        if (decodeReg[msgIdx / 8, msgIdx % 8] == Encoding.ASCII) addMsgAscii += GetMsgFromByte(msgToDecode[msgIdx], Encoding.ASCII);
                        else
                        {
                            addMsgAscii = "";
                            break;
                    ...
                    ret += addMsgAscii + "(>)" + Environment.NewLine;
                }
...
//CONCATESTART
        ...
        public static string CDDFILE_ECU_Core_Assembly_Part_Number (byte[] recvByteArrToDecode , params object[] argvs) 
         {
